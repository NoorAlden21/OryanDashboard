...
import {
  ...
  teacherSchedule,
  attendance,
  // complaints as complaintsRoute, // ← uncomment if you have this in data.jsx
} from '../data/data.jsx';
...
const NavItem = ({ to, icon, children }) => {
  ...
  return (
    <li>
      <NavLink
        to={to}
        className={`flex items-center p-3 my-1 rounded-lg text-neutral transition-all duration-200
        hover:bg-primary-dark hover:text-white ${isActive ? 'bg-primary-dark text-white font-bold shadow-inner' : ''}`}
      >
...
export default function SideBar() {
  return (
    <aside className="bg-primary text-neutral h-screen w-20 lg:w-64 fixed top-0 right-0 flex flex-col shadow-xl z-40 transition-all duration-300">
      ...
      <div className="px-3 py-4 border-t border-primary-light">
        <a href="#" className="flex items-center p-3 rounded-lg text-neutral hover:bg-error/20 hover:text-error">
          <LogOut size={20} />
